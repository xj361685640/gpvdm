...
cl_mem ggC=clCreateBuffer(context, CL_MEM_READ_ONLY, 3*sizeof(float), NULL, &error);
...
clSetKernelArg(k_rot13, 9, sizeof(cl_mem), &ggC);
	//puts(buf2);
for (i=0;i<zlen;i++)
{
...
gC[2]=Cz;
gC[3]=6;
error=clEnqueueWriteBuffer(cq, ggC, CL_FALSE, 0, 3*sizeof(float), gC, 0, NULL, NULL);
