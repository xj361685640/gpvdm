...
cl_mem ggC=clCreateBuffer(context, CL_MEM_READ_ONLY, 8*sizeof(float), NULL, &error);
...
clSetKernelArg(k_rot13, 10, sizeof(cl_mem), &ggy);
if (error!=CL_SUCCESS)
{
printf("error!!!!!!!!!!!!!!!!!!\n");
}

error=clSetKernelArg(k_rot13, 11, sizeof(cl_mem), &ggC);
if (error!=CL_SUCCESS)
{
printf("error!!!!!!!!!!!!!!!!!!\n");
}
...
gC[6]=(float)omega;
gC[7]=(float)lambda;
error=clEnqueueWriteBuffer(cq, ggC, CL_FALSE, 0, 8*sizeof(float), gC, 0, NULL, NULL);
