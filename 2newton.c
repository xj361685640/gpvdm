...
//      G=in->G[i];
		...
		if ((in->kl_in_newton == TRUE) && (in->interfaceleft == TRUE)
			...
			in->Ti[pos] = i;
			in->Tj[pos] = i;
			in->Tx[pos] = (double)dphic_d;
			pos++;
		}

		in->Ti[pos] = i;
		in->Tj[pos] = i + in->ymeshpoints * (1);
		in->Tx[pos] = (double)dphidxic;
		pos++;

		in->Ti[pos] = i;
		in->Tj[pos] = i + in->ymeshpoints * (1 + 1);
		in->Tx[pos] = (double)dphidxipc;
		pos++;
		...
		in->Tx[pos] = (double)dJdxic;
		pos++;

		in->Ti[pos] = in->ymeshpoints * (1) + i;
		in->Tj[pos] = in->ymeshpoints * (1 + 1) + i;
		in->Tx[pos] = (double)dJdxipc;
		pos++;

		in->Ti[pos] = in->ymeshpoints * (1) + i;
		in->Tj[pos] = i;
		in->Tx[pos] = (double)dJdphic;
		pos++;

		//hole
		in->Ti[pos] = in->ymeshpoints * (1 + 1) + i;
		in->Tj[pos] = in->ymeshpoints * (1 + 1) + i;
		in->Tx[pos] = (double)dJpdxipc;
		pos++;

		in->Ti[pos] = in->ymeshpoints * (1 + 1) + i;
		in->Tj[pos] = in->ymeshpoints * (1) + i;
		in->Tx[pos] = (double)dJpdxic;
		pos++;

		in->Ti[pos] = in->ymeshpoints * (1 + 1) + i;
		in->Tj[pos] = i;
		in->Tx[pos] = (double)dJpdphic;
		pos++;
		...
		if (in->ntrapnewton == TRUE) {
			for (band = 0; band < in->srh_bands; band++) {
				...
				in->Tj[pos] =
				    in->ymeshpoints * (1 + 1 + 1 + band) + i;
				in->Tx[pos] = (double)dntrapdntrap[band];
				pos++;

				in->Ti[pos] =
				    in->ymeshpoints * (1 + 1 + 1 + band) + i;
				in->Tj[pos] = in->ymeshpoints * 1 + i;
				in->Tx[pos] = (double)dntrapdn[band];
				pos++;

				in->Ti[pos] =
				    in->ymeshpoints * (1 + 1 + 1 + band) + i;
				in->Tj[pos] = in->ymeshpoints * (1 + 1) + i;
				in->Tx[pos] = (double)dntrapdp[band];
				pos++;

				in->Ti[pos] = in->ymeshpoints * (1) + i;
				in->Tj[pos] =
				    in->ymeshpoints * (1 + 1 + 1 + band) + i;
				in->Tx[pos] = (double)dJdtrapn[band];
				pos++;

				in->Ti[pos] = in->ymeshpoints * (1 + 1) + i;
				in->Tj[pos] =
				    in->ymeshpoints * (1 + 1 + 1 + band) + i;
				in->Tx[pos] = (double)dJpdtrapn[band];
				pos++;

				in->Ti[pos] = i;
				in->Tj[pos] =
				    in->ymeshpoints * (1 + 1 + 1 + band) + i;
				in->Tx[pos] = (double)dphidntrap[band];
				pos++;
			...
			for (band = 0; band < in->srh_bands; band++) {
				...
				in->Tj[pos] =
				    in->ymeshpoints * (1 + 1 + 1 +
						       in->srh_bands + band) +
				    i;
				in->Tx[pos] = (double)dptrapdptrap[band];
				pos++;
				...
				in->Ti[pos] =
				    ...
				    i;
				in->Tj[pos] = in->ymeshpoints * (1 + 1) + i;
				in->Tx[pos] = (double)dptrapdp[band];
				pos++;
				...
				in->Ti[pos] =
				    ...
				    i;
				in->Tj[pos] = in->ymeshpoints * (1) + i;
				in->Tx[pos] = (double)dptrapdn[band];
				pos++;
				...
				in->Tj[pos] =
				    in->ymeshpoints * (1 + 1 + 1 +
						       in->srh_bands + band) +
				    i;
				in->Tx[pos] = (double)dJpdtrapp[band];
				pos++;
				...
				in->Tj[pos] =
				    in->ymeshpoints * (1 + 1 + 1 +
						       in->srh_bands + band) +
				    i;
				in->Tx[pos] = (double)dJdtrapp[band];
				pos++;
				...
				in->Tj[pos] =
				    in->ymeshpoints * (1 + 1 + 1 +
						       in->srh_bands + band) +
				    i;
				in->Tx[pos] = (double)dphidptrap[band];
				pos++;
			}
		...
		if (i != (in->ymeshpoints - 1)) {
			...
			in->Ti[pos] = in->ymeshpoints * (1) + i;
			in->Tj[pos] = in->ymeshpoints * (1) + i + 1;
			in->Tx[pos] = (double)dJdxir;
			pos++;

			in->Ti[pos] = i + in->ymeshpoints * (1);
			in->Tj[pos] = i + 1;
			in->Tx[pos] = (double)dJdphir;
			pos++;

			//hole
			in->Ti[pos] = in->ymeshpoints * (1 + 1) + i;
			in->Tj[pos] = in->ymeshpoints * (1 + 1) + i + 1;
			in->Tx[pos] = (double)dJpdxipr;
			pos++;

			in->Ti[pos] = i + in->ymeshpoints * (1 + 1);
			in->Tj[pos] = i + 1;
			in->Tx[pos] = (double)dJpdphir;
			pos++;
		...
		in->b[in->ymeshpoints * (1) + i] =
		    (double)-((Jnr - Jnl) / (dylh + dyrh) - Rtrapn - Rfree);
		...
		//getchar();
		in->b[in->ymeshpoints * (1) + i] -= (double)Gn;
		...
		in->b[in->ymeshpoints * (1 + 1) + i] =
		    (double)-((Jpr - Jpl) / (dylh + dyrh) + Rtrapp + Rfree);
		//printf("%le %le\n",Rtrapn,Rtrapp);

		in->b[in->ymeshpoints * (1 + 1) + i] -= (double)-Gp;

		if (in->ntrapnewton == TRUE) {
			for (band = 0; band < in->srh_bands; band++) {
				in->b[in->ymeshpoints * (1 + 1 + 1 + band) +
				      i] = (double)-(dntrap[band]);
			}
		}
