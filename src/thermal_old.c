...
for (i=0;i<in->ymeshpoints;i++)
{
	...
	Jn=in->Jn[i];
	Jp=in->Jp[i];
	in->He[i]=Q*(Ecr-Ecl)*Jn/dh+in->Habs[i]/2.0+Q*in->Eg[i]*(in->Rn[i]+in->Rp[i])/2.0/2.0;
	in->Hh[i]=Q*(Evr-Evl)*Jp/dh+in->Habs[i]/2.0+Q*in->Eg[i]*(in->Rn[i]+in->Rp[i])/2.0/2.0;
	//printf("%Le %Le\n",in->He[i],in->Hh[i]);
...
gdouble Hh=0.0;
	...
	do
	{
		...
		solver(sim,thermal_M,thermal_N,thermal_Ti,thermal_Tj, thermal_Tx,thermal_b);
