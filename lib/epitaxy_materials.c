...
for (i=0;i<in->layers;i++)
{
	...
	{
		join_path(3, file_path,get_materials_path(sim),in->mat_file[i],"alpha_gen.omat");
	}else
	{

		join_path(3, file_path,get_materials_path(sim),in->mat_file[i],"alpha.omat");
	...
	{
		join_path(3, file_path,get_materials_path(sim),in->mat_file[i],"n_gen.omat");
	}else
	{
		join_path(3,file_path,get_materials_path(sim),in->mat_file[i],"n.omat");
		if (isfile(file_path)!=0)
		{
			join_path(3, file_path,get_materials_path(sim),in->mat_file[i],"n_gen.omat");
		}
